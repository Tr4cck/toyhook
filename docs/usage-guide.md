...
typedef struct {
    ...
    uint64_t duration;      // 耗时（ENTER 为 0，LEAVE 为与匹配 ENTER 之间的间隔）
    uint64_t hook_id;
    uint64_t thread_id;
    uint64_t args[TOY_MAX_ARGS];
    uint32_t argc;
    uint64_t ret_val;
    uint32_t kind;          // TOY_TRACE_ENTER 或 TOY_TRACE_LEAVE
} trace_event_t;
```
...
```c
toy_tracer_dump(tracer, fileno(stderr));   // 导出到任意 fd
```
